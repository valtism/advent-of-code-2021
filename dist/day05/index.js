...
const part1 = (rawInput) => {
  ...
  const horOrVert = input.filter((row) => row.start.x === row.end.x || row.start.y === row.end.y);
  const lines = horOrVert.flatMap((row) => {
    if (row.start.x === row.end.x) {
      ...
      if (row.start.y < row.end.y) {
        return Array(yDiff + 1).fill(null).map((_, i) => {
      ...
      if (row.start.x < row.end.x) {
        return Array(xDiff + 1).fill(null).map((_, i) => {
...
const part2 = (rawInput) => {
  ...
  input.forEach(({ start, end }) => {
    ...
    return Array(Math.abs(diff) + 1).fill(null).forEach((_, i) => {
