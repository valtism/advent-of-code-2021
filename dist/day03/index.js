...
function getCounts(list) {
  ...
  const counts = Array(stringLength).fill(null).map(() => ({ "0": 0, "1": 0 }));
  list.forEach((line) => {
...
const part2 = (rawInput) => {
  const input = parseInput(rawInput);
  const oxygenRating = getRating(input, (ones, zeros) => ones >= zeros ? "1" : "0");
  const co2Rating = getRating(input, (ones, zeros) => ones < zeros ? "1" : "0");
  return oxygenRating * co2Rating;
};
function getRating(input, comparison) {
...
function getOccurance(list, comparison) {
