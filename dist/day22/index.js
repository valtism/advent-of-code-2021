...
function parseInput(rawInput) {
  return rawInput.split("\n").map((line) => {
    ...
    const [xStart, xEnd, yStart, yEnd, zStart, zEnd] = line.match(/(-?\d+)/g).map(Number);
    return {
      on,
      xStart,
      xEnd,
...
function filterInput(input, bounds) {
  ...
  })).filter((cube) => !(cube.xStart > bounds.xEnd || cube.xEnd < bounds.xStart || cube.yStart > bounds.yEnd || cube.yEnd < bounds.yStart || cube.zStart > bounds.zEnd || cube.zEnd < bounds.zStart));
}
...
function intersectCubes(origin, intersecting) {
...
function cubeValue({ on, xStart, xEnd, yStart, yEnd, zStart, zEnd }) {
  const sign = on ? 1 : -1;
