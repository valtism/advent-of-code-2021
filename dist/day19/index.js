...
const parseInput = (rawInput) => rawInput.split("\n\n").map((scannerRaw) => {
  ...
  const beacons = lines.slice(1).map((line) => line.trim().split(",").map(Number));
  const beaconsWithRelativeDistances = beacons.map((beacon) => {
    const relativeDistances = beacons.map((otherBeacon) => {
      const distance = subtract(beacon, otherBeacon).map(Math.abs).sort();
...
function getTransforms(scanners) {
  const transforms = {};
  for (const scanner1 of scanners) {
    for (const scanner2 of scanners) {
      ...
      if (transform) {
        transforms[scanner1.id] = __spreadProps(__spreadValues({}, transforms[scanner1.id]), {
...
function matchScanners(scanner1, scanner2) {
...
function getBeaconTransform(beacon1, beacon2) {
  const distanceMatches = getDistanceMatches(beacon1.relativeDistances, beacon2.relativeDistances);
  if (distanceMatches.length < 12)
    return null;
  return getTransformFromMatches(distanceMatches);
}
function getDistanceMatches(list1, list2) {
  let distanceMatches = [];
...
function getTransformFromMatches(distanceMatches) {
  for (const rotate of rotations2) {
    const diff = subtract(distanceMatches[0].to, rotate(distanceMatches[0].from));
    const fits = distanceMatches.every((match) => isEqual(match.to, add(rotate(match.from), diff)));
    if (fits) {
...
function getAllPoints(scanners, transforms) {
  const vistied = new Set();
  const allPoints = new Set();
  function recurse(id, transform) {
    if (vistied.has(id))
      return;
    vistied.add(id);
...
function getMaxDistance(sensors) {
