...
const part1 = (rawInput) => {
  ...
  const unmarked = getUnmarked(winningBoard[0], drawn).reduce((acc, curr) => acc + curr, 0);
  return unmarked * drawn[drawn.length - 1];
};
function checkBoardForWin(board, drawn) {
...
function getUnmarked(board, drawn) {
  const unmarked = [];
  for (let x = 0; x < board.length; x++) {
    for (let y = 0; y < board.length; y++) {
      const num = board[y][x];
...
const part2 = (rawInput) => {
  ...
  const unmarked = getUnmarked(lastUnWonBoard, drawn).reduce((acc, curr) => acc + curr, 0);
  return unmarked * drawn[drawn.length - 1];
};
run({
  part1: {
